...
When $k=1$, the cost function is linear and not strictly convex, and the optimal transport plan is not unique. Any linear parameterization of the line segment connecting $x$ and $y$ will yield the same cost.

When $k\geq 2$, the cost function is strictly convex, and the optimal transport plan is unique. The optimal path is the straight line connecting $x$ and $y$ with velocity parameterized by the arc length.
...
$$
c(x,y)=\inf C_{\text{path}}=\int_0^1 c(\dot\gamma_t) dt = \int_0^1 v_t(x) dt = \frac{1}{2}\|y-x\|_2^2
$$
...
The $1$-rectified flow is calculated by:
$$
\begin{align*}
v_t^1(x) = \mathbb{E}_{(x_0,x_1)\sim \pi_0} \left[x_1-x_0 | x = t x_0 + (1-t) x_1\right]
\end{align*}
$$
where $\pi_0 = \mu \otimes \nu$ is the independent coupling of the source and target distributions.
This is the expectation of line direction that pass $x$ at time $t$.

For $k\geq 2$, the $k$-rectified flow is calculated by:
$$
\begin{align*}
v_t^k(x) = \mathbb{E}_{(x_0,x_1)\sim \pi_{k-1}} \left[x_1-x_0 | x = t x_0 + (1-t) x_1\right]
\end{align*}
$$
where $\pi_{k-1} =\pi_{g^{k-1}_1} = (\text{Id}, g_1^{k-1})_* \mu$ is the coupling induced by the $(k-1)$-rectified flow.

$$
C(\pi)=\mathbb{E}_{(x_0,x_1)\sim \pi} \left[c(x_0,x_1)\right]
$$
as the transport cost of the coupling $\pi$.

We assume the cost function has the form $c(x,y)=c(y-x)$ and $c$ is convex and $C^1$, we know the cost can be represented as the infimum of the path cost.
 
We has the following transform sequence:
$$
\pi_0=\mu \otimes \nu \xrightarrow{\Phi} g^1 \xrightarrow{\Psi} \pi_1 \xrightarrow{\Phi} g^2 \xrightarrow{\Psi} \cdots \xrightarrow{\Phi} g^{k} \xrightarrow{\Psi} \pi_{k}
$$
where $\Psi$ is pushforward $\Psi(g)=(\text{Id}, g)_*\mu$.

For $\Phi$, given $\pi$, $v$ is calculated by:
$$
v_t(x) = \mathbb{E}_{(x_0,x_1)\sim \pi} \left[x_1-x_0 | x = t x_0 + (1-t) x_1\right]
$$
And $g$ is determined by the following equation:
$$
\begin{cases}
\frac{d}{dt}g_t(x) = v_t(g_t(x)) \\
g_0(x)=x.
...
$$
\begin{align*}
C(\pi_{k+1}) &= \mathbb{E}_{(x_0,x_1)\sim \pi_{k+1}} \left[c(x_1-x_0)\right]\\
&=\mathbb{E}_{(x_0,x_1)\sim \pi_{k+1}} \left[c \left(\int_0^1 v_t^{k+1}(g_t^{k+1}(x_0))\right) dt\right] \\
&\leq \mathbb{E}_{(x_0,x_1)\sim \pi_{k+1}} \left[\int_0^1 c\left(v_t^{k+1}(g_t^{k+1}(x_0))\right) dt\right] \quad \text{(by Jensen's inequality)}\\
&= \int_0^1 \mathbb{E}_{(x_0,x_1)\sim \pi_{k+1}} \left[c\left(v_t^{k+1}(g_t^{k+1}(x_0))\right)\right] dt\\
&= \int_0^1 \mathbb{E}_{x_t \sim \rho_t^{k+1}} \left[c\left(v_t^{k+1}(x_t)\right)\right] dt\\
&= \int_0^1 \mathbb{E}_{x_t \sim \rho_t^k} \left[c\left(v_t^{k+1}(x_t)\right)\right] dt\\
&= \mathbb{E}_{(x_0,x_1)\sim \pi_k} \left[\int_0^1 c\left(v_t^{k+1}(x_t)\right) dt\right]\\
&= \mathbb{E}_{(x_0,x_1)\sim \pi_k} \left[\int_0^1 c\left(\mathbb{E}[x_1-x_0|x_t] \right) dt\right]\\
&\leq \mathbb{E}_{(x_0,x_1)\sim \pi_k} \left[\int_0^1 \mathbb{E}[c(x_1-x_0)|x_t] dt\right] \quad \text{(Jensen)}\\
&= \mathbb{E}_{(x_0,x_1)\sim \pi_k} \left[c(x_1-x_0)\right] \\
&= C(\pi_k)
\end{align*}
$$

Note that $\rho_t$ as the marginal distribution of $x_t = t x_1 + (1-t)x_0$, $\rho_{t}'$ is the distribution generated by the marginal vector field $v_t^{k+1}$. They are the same since the continuity equation is satisfied.

**Proof**

For all $\phi\in C_0(\Omega)$, 
...
Define
$$
S(g)=\int_0^1 \mathbb{E}_{(x_0,x_1)\sim \pi_{g_1}} \left[\|x_1-x_0-v_t(x_t)\|^2\right] dt
$$
as the straightness of the flow $g$. 

Theorem 3.7 in [Flow Straight and Fast](https://arxiv.org/abs/2209.03003) states:
$$
\min_{k\in\{1,\cdots,K\}} S(Z^k) \leq \frac{\mathbb{E}_{(x_0,x_1)\sim \pi_0}[\|x_1-x_0\|^2]}{K}
$$